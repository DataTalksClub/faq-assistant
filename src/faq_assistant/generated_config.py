# Generated by scripts/compile_config.py from config.toml.
# Do not edit by hand.

CONFIG = {'answering': {'include_sources': True,
               'max_output_tokens': 1200,
               'max_sources': 5,
               'prompts': {'course': 'You are a DataTalks.Club teaching assistant. Answer using '
                                     'only the retrieved\n'
                                     'context for the detected course. If the context does not '
                                     'answer the question,\n'
                                     'say that you could not find the answer in the course '
                                     'materials.\n',
                           'docs': 'You are a DataTalks.Club assistant. Answer using only the '
                                   'retrieved\n'
                                   'DataTalks.Club documentation context. If the context does not '
                                   'answer the\n'
                                   'question, say that you could not find the answer in the '
                                   'docs.\n'},
               'temperature': 0.2},
 'app': {'environment': 'dev', 'name': 'faq-assistant'},
 'cloudflare': {'account_id_env': 'CLOUDFLARE_ACCOUNT_ID',
                'ai': {'binding': 'AI',
                       'chat_model': '@cf/meta/llama-3.3-70b-instruct-fp8-fast',
                       'embedding_model': '@cf/baai/bge-base-en-v1.5'},
                'api_token_env': 'CLOUDFLARE_API_TOKEN',
                'vectorize': {'binding': 'FAQ_INDEX',
                              'dimensions': 768,
                              'index_name': 'faq-assistant-index',
                              'metadata_indexes': [{'property_name': 'scope', 'type': 'string'},
                                                   {'property_name': 'course', 'type': 'string'}],
                              'metric': 'cosine'},
                'worker': {'compatibility_date': '2026-06-17',
                           'compatibility_flags': ['python_workers'],
                           'name': 'faq-assistant'}},
 'courses': {'ai-dev-tools-zoomcamp': {'docs_prefix': 'courses/ai-dev-tools-zoomcamp/',
                                       'faq_enabled': True,
                                       'github_repositories': [{'ref': 'main',
                                                                'repo': 'DataTalksClub/ai-dev-tools-zoomcamp'}],
                                       'name': 'AI Dev Tools Zoomcamp',
                                       'slack_channels': ['C_AI_DEV_TOOLS']},
             'data-engineering-zoomcamp': {'docs_prefix': 'courses/data-engineering-zoomcamp/',
                                           'faq_enabled': True,
                                           'github_repositories': [{'ref': 'main',
                                                                    'repo': 'DataTalksClub/data-engineering-zoomcamp'}],
                                           'name': 'Data Engineering Zoomcamp',
                                           'slack_channels': ['C_DATA_ENGINEERING']},
             'llm-zoomcamp': {'docs_prefix': 'courses/llm-zoomcamp/',
                              'faq_enabled': True,
                              'github_repositories': [{'ref': 'main',
                                                       'repo': 'DataTalksClub/llm-zoomcamp'}],
                              'name': 'LLM Zoomcamp',
                              'slack_channels': ['C_LLM']},
             'machine-learning-zoomcamp': {'docs_prefix': 'courses/ml-zoomcamp/',
                                           'faq_enabled': True,
                                           'github_repositories': [{'ref': 'main',
                                                                    'repo': 'DataTalksClub/machine-learning-zoomcamp'}],
                                           'name': 'Machine Learning Zoomcamp',
                                           'slack_channels': ['C_ML']},
             'mlops-zoomcamp': {'docs_prefix': 'courses/mlops-zoomcamp/',
                                'faq_enabled': True,
                                'github_repositories': [{'ref': 'main',
                                                         'repo': 'DataTalksClub/mlops-zoomcamp'}],
                                'name': 'MLOps Zoomcamp',
                                'slack_channels': ['C_MLOPS']},
             'stock-markets-analytics-zoomcamp': {'docs_prefix': 'courses/stock-markets-analytics-zoomcamp/',
                                                  'faq_enabled': True,
                                                  'github_repositories': [{'ref': 'main',
                                                                           'repo': 'DataTalksClub/stock-markets-analytics-zoomcamp'}],
                                                  'name': 'Stock Markets Analytics Zoomcamp',
                                                  'slack_channels': []}},
 'ingestion': {'batch_size': 100,
               'chunk': {'max_chars': 1800, 'overlap_chars': 150},
               'github': {'fetch_library': 'gitsource', 'token_env': 'GITHUB_TOKEN'},
               'metadata_text_max_chars': 6000},
 'retrieval': {'default_limit': 6,
               'max_limit': 10,
               'min_score': 0.2,
               'rewrite_query': True,
               'scopes': {'course': {'filter_fields': {'course': 'required', 'scope': 'course'}},
                          'docs': {'filter_fields': {'scope': 'docs'}}}},
 'slack': {'bot_token_env': 'SLACK_BOT_TOKEN',
           'channels': {'C_AI_DEV_TOOLS': {'course': 'ai-dev-tools-zoomcamp',
                                           'name': 'course-ai-dev-tools',
                                           'scope': 'course'},
                        'C_DATA_ENGINEERING': {'course': 'data-engineering-zoomcamp',
                                               'name': 'course-data-engineering',
                                               'scope': 'course'},
                        'C_LLM': {'course': 'llm-zoomcamp',
                                  'name': 'course-llm',
                                  'scope': 'course'},
                        'C_ML': {'course': 'machine-learning-zoomcamp',
                                 'name': 'course-ml',
                                 'scope': 'course'},
                        'C_MLOPS': {'course': 'mlops-zoomcamp',
                                    'name': 'course-mlops',
                                    'scope': 'course'}},
           'default_scope': 'docs',
           'events_path': '/slack/events',
           'reply': {'thread': True, 'unfurl_links': False, 'unfurl_media': False},
           'signing_secret_env': 'SLACK_SIGNING_SECRET'},
 'sources': {'course_markdown': {'course_definitions': 'courses',
                                 'enabled': True,
                                 'github': {'exclude': ['vendor/**', 'node_modules/**', '.git/**'],
                                            'include': ['*.md', '**/*.md'],
                                            'include_from_course_docs_prefix': True,
                                            'ref': 'main',
                                            'repo': 'DataTalksClub/docs'},
                                 'item_id_prefix': 'course-md',
                                 'scope': 'course'},
             'course_repositories': {'course_definitions': 'courses',
                                     'enabled': True,
                                     'github': {'exclude': ['.git/**',
                                                            'data/**',
                                                            'images/**',
                                                            'notebooks/**',
                                                            'node_modules/**'],
                                                'include': ['README.md',
                                                            'cohorts/**/*.md',
                                                            'certificates.md',
                                                            'project.md',
                                                            'homework.md',
                                                            '**/*.md']},
                                     'item_id_prefix': 'repo',
                                     'scope': 'course'},
             'docs': {'enabled': True,
                      'github': {'exclude': ['courses/**',
                                             'vendor/**',
                                             'node_modules/**',
                                             '.git/**'],
                                 'include': ['*.md', '**/*.md'],
                                 'ref': 'main',
                                 'repo': 'DataTalksClub/docs'},
                      'item_id_prefix': 'docs',
                      'scope': 'docs'},
             'faq': {'courses_url': 'https://datatalks.club/faq/json/courses.json',
                     'enabled': True,
                     'item_id_prefix': 'faq'}}}
